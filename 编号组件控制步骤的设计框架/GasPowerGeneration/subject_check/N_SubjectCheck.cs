...
namespace GasPowerGeneration
{
     ...
     /// 高亮类型不计入统计分数里面
     /// </summary>
     public class N_SubjectCheck : AB_SubjectCheck
     {
          //步骤检查完成后的反馈
          Action m_callback;
          //选择的模式
          ...
          string m_startSoundName, m_endSoundName;
          /// <summary>
          /// 开始结束的语音
          /// </summary>
          ...
          private void fnp_TestCheck()
          {
               ...
               for (int i = 0; i < m_steps.Count; i++)
               {
                    ...
                    {//交互类型
                         if (m_steps[i].m_condition == "")
                         {
                              if (m_steps[i].m_state == mi_CheckValue.fni_checkDate(m_steps[i].m_id))
                              {//场景中状态和配置表状态一直，通过
                                   m_testRusult[i] = true;
                                   continue;
                              }
