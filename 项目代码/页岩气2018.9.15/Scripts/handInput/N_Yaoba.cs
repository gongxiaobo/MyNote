...
namespace GasPowerGeneration
{
     ...
     public class N_Yaoba : N_PickUp
     {
          ...
          [Tooltip("是否在最大值时才能去除摇把")]
          /// <summary>
          /// 是否在最大值时才能去除摇把
          /// </summary>
          public bool m_GetOutReachBig = false;
          public override void fn_handInOut(bool _inOut)
          {
               ...
               if (m_rotateControl && _inOut && (m_GetOutReachBig ? m_spanner .fni_valueToBig(): m_spanner.fni_valueToEdge()))
               {

                    Debug.Log("<color=blue>碰触到摇把，但是没有做摇入摇出动作，在等待2s钟为取出:</color>");
