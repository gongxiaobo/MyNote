...
namespace GasPowerGeneration
{
     ...
     public class S_ConnectWire : GenericSingletonClass<S_ConnectWire>, I_ConnectWireInit
     {
          ...
          /// <summary>
          /// 新的初始化方式，主要用于故障模式下的初始化线
          /// </summary>
          public void fn_initWireNew(string _state)
          {
               //如果接口的值已经和

               //检查一个接口多根连线的情况
               short t_num = S_ParseWirePara.fn_getParaNumber(_state);
               string[] t_alreadyConnect = S_ParseWirePara.fn_SplitToThree(_state);
               if (t_num==0)
               {
                    Debug.Log("<color=blue>不完整的消息链接信息</color>");
                    return;
               }
               if (t_num==1)
               {

                    if (!fnp_checkPipeCreated(t_alreadyConnect[0]))
                    {
                         fnp_codeConnect(t_alreadyConnect[0]); 
                    }
                    return;
               }
               if (t_num==2)
               {
                    ...
                    if (!fnp_checkPipeCreated(t_alreadyConnect[1]))
                    {
                         fnp_codeConnect(t_alreadyConnect[1]); 
                    }
                    return;
               }
               if (t_num==3)
               {
                    ...
                    if (!fnp_checkPipeCreated(t_alreadyConnect[2]))
                    {
                         fnp_codeConnect(t_alreadyConnect[2]); 
                    }
                    return;
               }
          ...
          /// <returns></returns>
          protected bool fnp_checkPipeCreated(string _state)
          {
               //先判断是否管线已经链接
               string[] t_info = S_ParseWirePara.fn_getPara(_state);
               string t_name = S_static.fns_OrderUp(t_info[0], t_info[1]);
               //判断电线已经存在
          ...
          /// 一条连接信息，链接两个点
          /// </summary>
          /// <param name="t_connect"></param>
          private void fnp_codeConnect(string t_connect)
          {
               S_selectUI.Instance.fni_setWireSelect(t_connect);
               
               //Debug.Log("<color=blue>开始链接</color>");
     
               string[] t_co = S_ParseWirePara.fn_getPara(t_connect);
          ...
          public void fn_select(AB_Name _select)
          {
               ...
               {
                    m_select1 = _select;
                    fnp_hightlightSelect(m_select1.gameObject, E_lightObjColor.e_yellow);
               }
               else
               {
          ...
          protected void fnp_newconnect(GameObject _slt1, GameObject _slt2, string _setting)
          {
               ...
               //Debug.Log("<color=blue>start new connect </color>" + _slt1.name + ":" + _slt2.name);
               ...
               {
                    Debug.Log("<color=red>over connect limit !</color>" + t_name);
                    return;
               }
               ...
               fnp_ScaleRadiu(ref t_size);
               Debug.Log("<color=blue>wire size </color>" + t_size.ToString());
               t_size =Mathf.Sqrt( t_size / 3.1415f)/1000f+0.001f;
               ...
               if (t_birthPipe.GetComponent<AB_LightOneObj>() == null)
               {
          ...
          private void fnp_SetSelectPort(string t_para, GameObject _port)
          {
               ...
               {//已经有链接线的情况
                    bool t_have = false;
                    //先判断是否包含新的链接线,这里主要是为了故障模式初始化电线设置接口值的问题
                    string[] t_alreadyConnect = S_ParseWirePara.fn_SplitToThree(t_state1);
                    for (int i = 0; i < 3; i++)
                    {
                         if (t_alreadyConnect[i]=="")
                         {
                              continue;
                         }
                         if (t_alreadyConnect[i]==t_para)
                         {
                              t_have = true;
                              break;
                         }
          ...
          protected void fnp_ScaleRadiu(ref float _r)
          {
               if (_r==0.5f)
               {
                    _r = 1.5f;
