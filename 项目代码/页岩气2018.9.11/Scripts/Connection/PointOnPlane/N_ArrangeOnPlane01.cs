...
//#endif
          ...
          protected override void fnp_findPos(ref Vector3[] _rec)
          {
               ...
               {//
                    ...
                    t_returnCheck=fnp_checkAndMove(ref _rec, item);
                    if (t_returnCheck)
                    {//检查到矩形相交，需要停止当前的检查
                         break;
                    }
          ...
          protected virtual bool fnp_checkAndMove(ref Vector3[] _rec, AB_Rectangle item)
          {
               Vector3[] t_oldRec = item.fn_get4Point();
               float t_movedis=0f;
               if (s_PointInRectangle.fns_RectInter(_rec,t_oldRec,out t_movedis,false,0.1f))
               {//表示矩形相交
