...
namespace GasPowerGeneration
{
     ...
     {
          public bool m_setAngelWithMix = false;
          /// <summary>
          ...
          public override void OnTriggerEnter(Collider other)
          {
               ...
               if (other.tag == "yaoba")
               {
                         ...
                         if (t_value != "")
                         {
                              ...
                              {//设置把手的旋转角度值
                                   ...
                                   {//这里使用的最大值，这种使用只能在【0，max】这种范围下才有效
                                        t_spanner.fn_setTo(t_rt.M_getRange.y * (t_setvalue > 1f ? 1f : t_setvalue));

                                        //Debug.Log("<color=blue>t_rt.M_getRange.y * t_setvalue=</color>" +
                                        //     t_rt.M_getRange.y * t_setvalue);
                    ...
                    if (m_setAngelWithMix)
                    {
                         ...
                         //     //Debug.Log("<color=blue>t_rt.M_getRange.x=</color>" + t_spanner.fni_getLimit().x);
