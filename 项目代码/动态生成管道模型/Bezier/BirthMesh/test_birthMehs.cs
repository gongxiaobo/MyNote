...
public class test_birthMehs : MonoBehaviour {
	...
	void Start () {
     ...
     {
          m_extrudeShape = new ExtrudeShape();
          test_circle t_circle = gameObject.AddComponent<test_circle>();
          t_circle.m_divid = (float)m_segment;
          t_circle.m_circleR = m_wireR;
          Vector3[] t_vectors = t_circle.fnp_caculateCircle();
          ...
          for (int i = 0; i < t_vectors.Length; i++)
          {
               ...
               //Debug.Log("<color=red>red:</color>" + i.ToString() + ":" + t_vectors[i]);
          }
          for (int i = 0; i < t_vectors.Length; i++)
          {
               m_extrudeShape.m_us[i] = i * t_offset;
               //Debug.Log("<color=red>red:</color>" + i.ToString() + ":" + m_extrudeShape.m_us[i]);
          }
     ...
     protected void fnp_CaculateHardMeshPoint()
     {
          ...
          for (int i = 0; i < m_Points.Count; i++)
          {
               ...
               {//添加点前后两个点
                    //float t_line1 = Vector3.Distance(t_points[i - 1], t_points[i]);
                    //float t_line2 = Vector3.Distance(t_points[i], m_Points[i + 1].transform.position);
                    //float t_offset = 0.05f;
                    //if (t_line1>=t_line2)
                    //{
                    //     t_offset = t_line2 * 0.05f;
                    //}
                    //else
                    //{
                    //     t_offset = t_line1 * 0.05f;
                    //}
                    //第一个点
                    //Vector3 t_one = S_BezierTool.fnp_VectorLerp(t_points[i - 1], t_points[i], 0.95f);
                    Vector3 t_one = S_BezierTool.fnp_VectorLerp(t_points[i - 1], t_points[i],1f- m_wireR/Vector3.Distance(t_points[i - 1], t_points[i]));
                    //Vector3 t_three = S_BezierTool.fnp_VectorLerp(t_points[i], m_Points[i + 1].transform.position, 1 - 0.95f);
                    Vector3 t_three = S_BezierTool.fnp_VectorLerp(t_points[i], m_Points[i + 1].transform.position, m_wireR / Vector3.Distance(t_points[i], m_Points[i + 1].transform.position));
